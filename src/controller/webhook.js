       ...
       // console.log("emrgency events",req.body.events[0])
        ...
        // }else{
            ...
            if(req.body.events[0].change.new_value.gid === '1204345653271174'){
                ...
                const timingStatus = customfield.find( item => item.gid === "1204345653271174");
                if(timingStatus.display_value === "Emergency" || timingStatus.display_value === "Exception" ){
                   startTimer(taskId);
                   let authordata;
                   let taskUrl;
                    firebase.getCurrentAuthor()
                    .then(async (result) => {
                        console.log("author data",result);
                        authordata = result;
                        await firebase.getTaskUrl(taskId);
                        await firebase.addTask(taskDetails.data.gid,taskDetails.data.name,date,authordata.Name);
...
exports.setionChange_Post = async (req,res) => {
 ...
 async function getTaskDetails(id) {
          ...
          headers: {
            Authorization: `Bearer 1/1204345610389960:556ef6643e0becdad45d82a6a77dae98`
          }
        });
